...
function expectKedaRequest() {
  expect(axios.get).toHaveBeenCalledWith(
    `http://kubernetes.default.svc/apis/keda.sh/v1alpha1/namespaces/${namespace}/scaledobjects/${scaledObjectName}`,
    { timeout: 5000 },
  );
}
...
describe('KEDAService.getScaledObjectStatus', () => {
  it('returns a validation error for a missing namespace', async () => {
    const result = await kedaService.getScaledObjectStatus(undefined, scaledObjectName);
  ...
  it.each([null, '', '   '])('rejects an empty namespace value: %p', async (emptyNamespace) => {
    const result = await kedaService.getScaledObjectStatus(emptyNamespace, scaledObjectName);
  ...
  it('trims and URL-encodes namespace and scaled object name', async () => {
    ...
    expect(axios.get).toHaveBeenCalledWith(
      'http://kubernetes.default.svc/apis/keda.sh/v1alpha1/namespaces/team%20namespace/scaledobjects/api%2Fobject',
      { timeout: 5000 },
    );
  });
  ...
  it.each([201, 202, 204, 400, 404, 500])(
    ...
    async (status) => {
      axios.get.mockResolvedValue({ status, data: { message: 'KEDA response' } });
  ...
  it('handles a timeout without throwing', async () => {
    axios.get.mockRejectedValue({ code: 'ECONNABORTED', message: 'timeout of 5000ms exceeded' });
...
describe('KEDAService.waitForScaledObjectStatus', () => {
  it('returns immediately when the first status satisfies the predicate', async () => {
    ...
    expect(predicate).toHaveBeenCalledWith(scaledObjectResponse.status, expect.any(Object));
  ...
  it('polls at the configured interval until the predicate succeeds', async () => {
    vi.useFakeTimers();
    axios.get
      .mockResolvedValueOnce({ status: 200, data: { status: { conditions: [] } } })
      .mockResolvedValueOnce({ status: 200, data: scaledObjectResponse });
    const predicate = vi.fn((status) => status.conditions?.some((condition) => condition.type === 'Ready'));
...
describe('KEDAService Prometheus helpers', () => {
  ...
  it('combines successful autoscaling metrics', async () => {
    vi.spyOn(kedaService, 'getAPIRequests').mockResolvedValue({ success: true, value: 60 });
    vi.spyOn(kedaService, 'getAPILatency').mockResolvedValue({ success: true, value: 120 });
    vi.spyOn(kedaService, 'getCPUUsage').mockResolvedValue({ success: true, value: 0.5 });
  ...
  it('fails autoscaling metrics when one provider fails', async () => {
    vi.spyOn(kedaService, 'getAPIRequests').mockResolvedValue({ success: true, value: 1 });
    vi.spyOn(kedaService, 'getAPILatency').mockResolvedValue({ success: false, error: 'latency unavailable' });
    vi.spyOn(kedaService, 'getCPUUsage').mockResolvedValue({ success: true, value: 0.2 });
    vi.spyOn(kedaService, 'getMemoryUsage').mockResolvedValue({ success: true, value: 0.2 });
    vi.spyOn(kedaService, 'getReplicaCount').mockResolvedValue({ success: true, value: 2 });
  ...
  it('returns metric failure details when recommendation inputs fail', async () => {
    ...
    await expect(kedaService.getScaleRecommendation(namespace, 'api')).resolves.toBe(failure);
  });
});
