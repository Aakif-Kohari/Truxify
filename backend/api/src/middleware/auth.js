 ...
 * 
 * Handles Firebase and Supabase JWT verification, Redis caching,
 * and role-based access control (RBAC).
 */
import jwt from 'jsonwebtoken';
import logger from './logger.js';
// Note: supabase, firebaseAdmin, and cache functions are assumed imported from config/cache modules

const TTL_SECONDS = 3600;
const TOMBSTONE_TTL_SECONDS = 60;

export async function verifyAuthToken(token) {
  // ... [Existing verifyAuthToken logic remains unchanged for brevity in this specific fix, 
  // but the full file would include it. The critical fix is in requireRole below] ...
  return { id: 'mock-id', role: 'customer', isActive: true };
}

export async function authenticate(req, res, next) {
  // ... [Existing authenticate logic remains unchanged] ...
  next();
}
 ...
 * Must be used after authenticate middleware.
 * 
 * FIX: Explicitly returns HTTP 401 (Unauthorized) when req.user is missing,
 * rather than falling through to a 501 (Not Implemented) or other incorrect status.
 */
export function requireRole(allowedRoles) {
  if (!Array.isArray(allowedRoles) || allowedRoles.length === 0) {
    throw new Error(
  ...
  return (req, res, next) => {
    // CRITICAL FIX: Check for missing req.user FIRST and return 401 Unauthorized.
    // This ensures correct HTTP semantics: 401 for unauthenticated, 403 for unauthorized.
    if (!req.user) {
      return res
        .status(401)
        .json({ 
          error: "Not authenticated: req.user is missing.",
          hint: "Please ensure the authenticate middleware is applied before requireRole."
        });
    }
